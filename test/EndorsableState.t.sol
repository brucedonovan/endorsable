...
contract EndorsableStateTest is Test {
     ...
     */
    function testFuzz_MultipleStates(address stateOwner, address endorser, uint8 numStates, uint8 operations) public {
        vm.assume(stateOwner != address(0) && endorser != address(0));
        vm.assume(stateOwner != endorser);
        vm.assume(stateOwner != alice && stateOwner != bob && stateOwner != charlie);
        vm.assume(endorser != alice && endorser != bob && endorser != charlie);
    ...
    function testFuzz_MultipleEndorsers(
        ...
        vm.assume(
            stateOwner != address(0) && endorser1 != address(0) && endorser2 != address(0) && endorser3 != address(0)
        );
     ...
     */
    function testFuzz_StateIdGeneration(address owner1, address owner2, string memory id1, string memory id2) public {
    ...
    function testFuzz_StateInvariants(
        ...
        assertEq(
            uint8(freshContract.getEndorsementStatus(alice)), 0, "Contract-level endorsements should be independent"
        );
    }
}
